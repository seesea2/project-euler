...
void Problem23() {
	...
	double sum = 0;							//rslt is: 4179871

	bool search_new_upper_limit = true;
	vector<int> abundant_num_list;
	for(int i = 1; i < kKnowUpperLimit; ++i)
	{
		if(IsAbundantNum(i) == true)
			abundant_num_list.push_back(i);
	}
	cout << "abundant_num_list.size : " << abundant_num_list.size() << endl;
	...
	while(num > 0)
	{
		...
		for(vector<int>::iterator it1 = abundant_num_list.begin(); it1 != abundant_num_list.end(); ++it1)
		{
			if(*it1 >= num)
				break;

			//use binary search to speed up the search
			size_t lower_ind = 0, upper_ind = abundant_num_list.size() - 1, ind = 0;
			if(*it1 + abundant_num_list[lower_ind] == num || *it1 + abundant_num_list[upper_ind] == num)
			{
			...
			else
			{
				while(1)
				{
					...
					if(*it1 + abundant_num_list[ind] == num)
					{
						sum_of_two_abundant = true;
						break;
					}
					else if(*it1 + abundant_num_list[ind] > num)
					{
						upper_ind = ind;
					}
		...
		if(search_new_upper_limit == true && sum_of_two_abundant == true)
		{
			new_upper_limit = num;
		}
	...
	cout << "Sum of all non_two_abundant_numbers : " << sum << endl << endl;
}
